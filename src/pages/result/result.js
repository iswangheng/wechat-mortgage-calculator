// \u8fd8\u6b3e\u660e\u7ec6\u9875\u9762
const { calculateCommercialLoan, calculateFundLoan, calculateCombinationLoan } = require('../../utils/mortgage-calculator');
const { drawPieChart, drawLineChart, drawBarChart, initCanvas } = require('../../utils/chart-utils');
...
Page({
  ...
  drawPaymentTrendChart() {
    ...
    const principalData = keyMonths.map((m) => parseFloat(schedule[m - 1].principal));
    const interestData = keyMonths.map((m) => parseFloat(schedule[m - 1].interest));

    initCanvas('#lineCanvas', this).then(({ canvas, ctx, width, height }) => {
  ...
  drawCumulativeBarChart() {
    ...
    initCanvas('#barCanvas', this).then(({ canvas, ctx, width, height }) => {
  ...
  generateSchedule() {
    ...
    if (method === '\u7b49\u989d\u672c\u606f') {
      const monthlyPayment = parseFloat(result.monthlyPayment.replace(/,/g, ''));
      const monthlyRate = this.calculateMonthlyRate(principal, monthlyPayment, totalMonths);
      ...
      const monthlyDecrease = ((firstPayment - lastPayment) / totalMonths).toFixed(2);
  ...
  calculateMonthlyRate(principal, monthlyPayment, totalMonths) {
    ...
    for (let i = 0; i < 50; i++) {
      const calculated = principal * (rate * Math.pow(1 + rate, totalMonths)) / (Math.pow(1 + rate, totalMonths) - 1);
  ...
  updateStatistics() {
    ...
    const totalPrincipalPaid = displaySchedule.reduce((sum, item) =>
      sum + parseFloat(item.principal), 0
    ).toFixed(2);

    const totalInterestPaid = displaySchedule.reduce((sum, item) =>
      sum + parseFloat(item.interest), 0
    ).toFixed(2);
  ...
  drawShareCard(ctx, W, H) {
    ...
    const monthlyValue = method === '等额本息' ? result.monthlyPayment : result.firstMonthPayment;
