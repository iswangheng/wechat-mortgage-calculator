...
Page({
  ...
  restoreUserPreferences() {
    ...
    if (lastYears) {
      ...
      const defaults =
        CITY_DEFAULTS[this.data.cityName] || CITY_DEFAULTS["default"];
      this.setData({
  ...
  validateField(field, value) {
    ...
    if (numVal < rule.min)
      return `${rule.label}不能低于${rule.min}${rule.unit}`;
    if (numVal > rule.max)
      return `${rule.label}不能超过${rule.max}${rule.unit}`;
    return "";
  },
  ...
  onApplyTemplate(e) {
    ...
    try {
      wx.reportAnalytics("use_template", { template_name: template.name });
    } catch (e) {
  ...
  onCalculate() {
    ...
    if (loanType === "commercial") {
      ...
      if (parseFloat(commercialPrincipal) > MAX_LOAN_AMOUNT) {
        wx.showToast({
          title: `商贷金额不能超过${MAX_LOAN_AMOUNT}万元`,
          icon: "none",
          duration: 2000,
        });
        return;
      ...
      if (
        !commercialRate ||
        parseFloat(commercialRate) < MIN_RATE ||
        parseFloat(commercialRate) > MAX_RATE
      ) {
        wx.showToast({
          title: `商贷利率必须在${MIN_RATE}%-${MAX_RATE}%之间`,
          icon: "none",
          duration: 2000,
        });
        return;
      ...
      if (
        !fundRate ||
        parseFloat(fundRate) < MIN_RATE ||
        parseFloat(fundRate) > MAX_RATE
      ) {
        wx.showToast({
          title: `公积金利率必须在${MIN_RATE}%-${MAX_RATE}%之间`,
          icon: "none",
          duration: 2000,
        });
        return;
      ...
      const totalPrincipal =
        parseFloat(commercialPrincipal) + parseFloat(fundPrincipal);
      if (totalPrincipal > MAX_LOAN_AMOUNT) {
        wx.showToast({
          title: `贷款总额不能超过${MAX_LOAN_AMOUNT}万元`,
          icon: "none",
          duration: 2000,
        });
        return;
    ...
    try {
      ...
      try {
        wx.reportAnalytics("calculate", {
          ...
          amount: parseFloat(this.data.commercialPrincipal || 0),
        });
      } catch (e) {
        /* ignore analytics error */
      }
      ...
      if (error.message) {
        if (error.message.includes("本金") || error.message.includes("金额")) {
          ...
          error.message.includes("年限") ||
          error.message.includes("月数")
        ) {
          errorMessage = error.message;
        } else if (
  ...
  formatResult(result) {
    ...
    if (formatted.monthlyPayment) {
      formatted.monthlyPayment = formatted.monthlyPayment.toLocaleString(
        "zh-CN",
        {
    ...
    if (formatted.firstMonthPayment) {
      formatted.firstMonthPayment = formatted.firstMonthPayment.toLocaleString(
        "zh-CN",
        {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
    ...
    if (formatted.lastMonthPayment) {
      formatted.lastMonthPayment = formatted.lastMonthPayment.toLocaleString(
        "zh-CN",
        {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
    ...
    if (formatted.totalInterest) {
      formatted.totalInterest = formatted.totalInterest.toLocaleString(
        "zh-CN",
        {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
  ...
  onViewDetail() {
    ...
    const annualRate =
      loanType === "fund" ? parseFloat(fundRate) : parseFloat(commercialRate);
  ...
  saveToHistory(formattedResult) {
    ...
    try {
      saveHistory({
        ...
        result: {
          monthlyPayment:
            formattedResult.monthlyPayment ||
            formattedResult.firstMonthPayment ||
            "",
