...
function calculateEarlyRepayment(params) {
  ...
  // 计算已还款后的剩余本金
  const remainingPrincipal = original.schedule[paidMonths - 1].remainingPrincipal;
  ...
  if (earlyAmount * 10000 > remainingPrincipal) {
    throw new Error(`提前还款金额不能超过剩余本金 ${(remainingPrincipal / 10000).toFixed(2)}万元`);
  }
  ...
  if (afterMethod === 'reduceTerm') {
    ...
    return {
      ...
      original: {
        ...
        monthlyPayment: method === '等额本金' ? original.firstMonthPayment : original.monthlyPayment,
      ...
      afterEarlyPayment: {
        ...
        monthlyPayment: method === '等额本金' ? newLoan.firstMonthPayment : newLoan.monthlyPayment,
        newTotalPayment: newLoan.totalPayment + original.schedule.slice(0, paidMonths).reduce((sum, m) => sum + m.monthlyPayment, 0),
        newTotalInterest: newLoan.totalInterest,
        savedMonths: remainingMonths - newMonths,
        savedInterest: original.totalInterest - newLoan.totalInterest - (paidMonths > 0 ? original.schedule.slice(0, paidMonths).reduce((sum, m) => sum + m.interestPayment, 0) : 0)
      }
    };
    ...
    // 保持年限不变，减少月供
    if (method === '等额本金') {
      newLoan = calculateEqualPrincipal(newPrincipal, annualRate, remainingYears);
    } else {
      newLoan = calculateEqualPayment(newPrincipal, annualRate, remainingYears);
    }

    const originalMonthly = method === '等额本金' ? original.firstMonthPayment : original.monthlyPayment;
    const newMonthly = method === '等额本金' ? newLoan.firstMonthPayment : newLoan.monthlyPayment;
 ...
 * @returns {object} 安全建议
 */
function calculateSafetyLine(monthlyPayment) {
  ...
  return {
    ...
    suggestion: monthlyPayment / comfortable <= 0.3 ? '✅ 舒适' :
                monthlyPayment / acceptable <= 0.4 ? '⚠️ 可接受' :
                '❌ 压力大'
  };
}
...
function comparePaymentMethods(principal, annualRate, years) {
  ...
  const savedInterest = equalPrincipal.totalInterest - equalPayment.totalInterest;
  const firstMonthDiff = equalPrincipal.firstMonthPayment - equalPayment.monthlyPayment;
  ...
  return {
    ...
    equalPrincipal: {
      ...
      pros: ['总利息更少', `能省 ${Math.round(savedInterest)}元`, '适合收入高或有余钱的人'],
    ...
    recommendation: firstMonthDiff / equalPayment.monthlyPayment < 0.2 ?
      '建议选等额本金，能省钱且首月压力不大' :
